...
- 2026-09-06 — گسترش additive از رزولوشن تیکت ۱۵ (پیش‌نمایش اقساط): `summaries` برای ماه‌های آیندهٔ جلالی ترکیبی است (ثبت‌شده + پیش‌بینی، با فیلد اختیاری `forecastToman`) + خواندن سطح‌آیتم جدید `GET /api/v1/recurring-templates/preview?month=`. شکل این تیکت برای ماه جاری/گذشته دست‌نخورده. جزئیات: تیکت ۱۵.

- 2026-09-06 — پیوست (از رزولوشن تیکت ۰۸): مرز auth وب/موبایل تثبیت شد — توکن سشن بعد از ورود در هدر `set-auth-token`، مصرف با `Authorization: Bearer` روی همهٔ `/api/v1`؛ بدون endpoint جداگانه برای موبایل. جزئیات سشن/ورود/ریست رمز: تیکت ۰۸.
